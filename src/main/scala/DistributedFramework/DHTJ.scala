...
object DHTJ {
  def main(args: Array[String]): Unit = {
    ...
    val segnDay_afcidTripIs = spark.read.parquet("/data/segNday2afcidNsetWithTripIndex").
      map(line => ((line(0).toString.toInt,line(1).toString.toInt),line(2).toString.split(";").map(v =>
        (v.split(":")(0).toInt,v.split(":")(1).split(",").map(_.toInt)))
      ))// ((segN,day), Array[(afcidN, Array[tripIndex])])
    val segnDay2afcidTripIs = sc.broadcast(segnDay_afcidTripIs.collect().toMap)
    ...
    APTrips.zipPartitions(AFCPatterns){ (apIter, afcIter)=>{
      ...
      while (apIter.hasNext) {
        ...
        val afcidN_rawOVTripPairs = apTrips.zipWithIndex.flatMap(apTrip_index=>{
          ...
          val afcidN2tripIs = paths.flatMap(path => {
            ...
            val afcidN2tripIs_path = ovsegNs.flatMap(ovsegN => segnDay2afcidTripIs.value.getOrElse((ovsegN, day), Array[(Int, Array[Int])]())).
              filter(v => afcidN2TripsPatterns.contains(v._1))
            afcidN2tripIs_path
        ...
        val afcs = afcidN_rawOVTripPairs.map(v => (v._1, getOVtripPairs(v._2,afcidN2TripsPatterns(v._1)._2.flatMap(x => x).toArray))).
          map(v => {
      ...
      res.iterator
    }}.toDF("apidN","afcidN","sim").write.mode("overwrite").parquet(s"/data/dhtj_MTP-Tree_simpair")
